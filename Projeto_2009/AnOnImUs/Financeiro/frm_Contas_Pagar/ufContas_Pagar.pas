...
uses
  ...
  frxDBSet, frxDesgn, frxExportPDF;

type
  TfContas_Pagar = class(TfBasico)
    panelFiltros: TPanel;
...
var
  fContas_Pagar: TfContas_Pagar;

implementation

uses udmContas_Pagar, ufCad_Contas_Pagar, ufMensagem;

{$R *.dfm}

procedure TfContas_Pagar.btnFiltrarClick(Sender: TObject);
begin
  inherited;
  OpenClientDataSet(1);
end;

function TfContas_Pagar.GeraFilter : String;
var
  strFilter : String;
begin
  strFilter := dmContas_Pagar.InstrucaoSQL;
  ...
  if edtDt_Pagto_Inicio.Date > 0 then
    strFilter := strFilter + ' AND CPAG_DTA_PGTO    >=    ' + QuotedStr(FormatDateTime('mm/dd/yyyy',edtDt_Pagto_Inicio.Date));
  if edtDt_Pagto_Fim.Date > 0 then
    strFilter := strFilter + ' AND CPAG_DTA_PGTO    <=    ' + QuotedStr(FormatDateTime('mm/dd/yyyy',edtDt_Pagto_Fim.Date));
  if edtDtEfetivaInicio.Date > 0 then
    strFilter := strFilter + ' AND CPAG_DTA_EFETIVO >=    ' + QuotedStr(FormatDateTime('mm/dd/yyyy',edtDtEfetivaInicio.Date));
  if edtDtEfetivaFim.Date > 0 then
    strFilter := strFilter + ' AND CPAG_DTA_EFETIVO >=    ' + QuotedStr(FormatDateTime('mm/dd/yyyy',edtDtEfetivaFim.Date));
  case rdStatus.ItemIndex of
    0 : strFilter := strFilter + ' AND CPAG_STATUS = ' + QuotedStr('A');
    1 : strFilter := strFilter + ' AND CPAG_STATUS = ' + QuotedStr('F');
    2 : strFilter := strFilter + ' AND CPAG_STATUS = ''C'' AND CPAG_PREVISAO = 0';
    4 : strFilter := strFilter + ' AND CPAG_STATUS = ''A'' AND CPAG_PREVISAO = 1';
  end;
  strFilter := strFilter + ' ORDER BY CP.CPAG_DTA_VCTO, CP.CPAG_STATUS';
  Result := strFilter;
end;

procedure TfContas_Pagar.BitBtn2Click(Sender: TObject);
var
  intI : Integer;
begin
...
procedure TfContas_Pagar.btnFlatClick(Sender: TObject);
begin
  inherited;
...
procedure TfContas_Pagar.btnNovoClick(Sender: TObject);
...
var
  ...
  intID  : Integer;
begin
  intID := cdsGeral.FieldByName('CPAG_ID').AsInteger;
      ...
      if ConfCancelamento then begin
        while strTMP = '' do begin
          ...
          if strTMP <> '' then begin
            GeraAcerto(cdsGeral.FieldByName('CPAG_PLCT_ID').AsString,
                       cdsGeral.FieldByName('CPAG_GENE_ID_TIPO_DOC').AsString,
                       cdsGeral.FieldByName('CPAG_ID').AsString,
                       cdsGeral.FieldByName('CPAG_VALOR_PAGO').AsFloat);
  ...
  OpenClientDataSet;
  cdsGeral.Locate('CPAG_ID',intID,[]);
end;

procedure TfContas_Pagar.CancelaTítulo(strMotivo : String);
begin
  with cdsGeral do begin
    if (FieldByName('CPAG_VALOR_PAGO').AsFloat > 0) and (FieldByName('CPAG_VIA_BANCO').AsInteger = 0) then begin
      funcBasico.LancamentoLivroCaixa('146',
                                      '3',
                                      ...
                                      '11',
                                      '3',
                                      FieldByName('CPAG_VALOR_PAGO').AsFloat);
    end else if (FieldByName('CPAG_VALOR_PAGO').AsFloat > 0) then begin
      funcBasico.RegistraMovBancario(FieldByName('CPAG_CBCO_ID').AsString,FieldByName('CPAG_VALOR_PAGO').AsFloat);
      funcBasico.RegistraMovConta(FieldByName('CPAG_CBCO_ID').AsString,'1003','1',FieldByName('CPAG_ID').AsString,FieldByName('CPAG_VALOR_PAGO').AsFloat);
    end;
    Edit;
    if FieldByName('CPAG_STATUS').AsString = 'A' then begin
      FieldByName('CPAG_STATUS').AsString    := 'C';
      if cdsGeral.FieldByName('CPAG_PREVISAO').AsInteger = 0 then begin
        funcBasico.BaixarContaReceberPrevisto(cdsGeral.FieldByName('CPAG_DTA_VCTO').AsDateTime,
                                              cdsGeral.FieldByName('CPAG_VALOR_DOC').AsFloat,
                                              cdsGeral.FieldByName('CPAG_PLCT_ID').AsString);
      end;
      FieldByName('CPAG_PREVISAO').AsInteger := 0;
    end else begin
      FieldByName('CPAG_STATUS').AsString    := 'A';
      FieldByName('CPAG_VALOR_PAGO').AsFloat := 0;
    end;
    FieldByName('CPAG_MOTIVO_CANC').AsString   := strMotivo;
    FieldByName('CPAG_DTA_CANC').AsDateTime    := funcBasico.DataServidor(funcBasico.Conexao);
    FieldByName('CPAG_USER_ID_CANC').AsInteger := funcBasico.User_ID;
    ApplyUpdates(-1);
  end;
end;

function TfContas_Pagar.ConfCancelamento : Boolean;
begin
  try
    Result := true;
    Carregadados;
    ...
    with fMensagem do begin
      ...
      lbProveniente.Caption := fCad_Contas_Pagar.ContaPagar.strProv;
      lbFornecedor.Caption  := fCad_Contas_Pagar.ContaPagar.strCod_Forn + ' - ' + fCad_Contas_Pagar.ContaPagar.strFornec;
      lbPlano.Caption       := funcBasico.Buscar_Coluna(fCad_Contas_Pagar.ContaPagar.strPlano,1) + ' - ' + funcBasico.Buscar_Coluna(fCad_Contas_Pagar.ContaPagar.strPlano,2);
      lbTipo_doc.Caption    := fCad_Contas_Pagar.ContaPagar.strCod_Doc  + ' - ' + fCad_Contas_Pagar.ContaPagar.strDoc;
      lbVlr.Caption         := 'R$ ' + FormatFloat('0.00',fCad_Contas_Pagar.ContaPagar.floatVlr);
      ShowModal;
      Result := boolConfirm;
    end;
  finally
...
procedure TfContas_Pagar.Carregadados;
begin
  with cdsGeral do begin
    fCad_Contas_Pagar.ContaPagar.strNr_Doc   := FieldByName('CPAG_NR_DOC').AsString;
    fCad_Contas_Pagar.ContaPagar.strProv     := FieldByName('CPAG_PROVENIENTE').AsString;
    fCad_Contas_Pagar.ContaPagar.strCod_Forn := FieldByName('CPAG_FORN_ID').AsString;
    fCad_Contas_Pagar.ContaPagar.strFornec   := FieldByName('FORN_NOME_FANTASIA').AsString;
    fCad_Contas_Pagar.ContaPagar.strCod_Plct := FieldByName('CPAG_PLCT_ID').AsString;
    fCad_Contas_Pagar.ContaPagar.strPlano    := FieldByName('PLCT_COD_SIMPLIFICADO').AsString + ';' + FieldByName('PLCT_DESCRICAO').AsString;
    fCad_Contas_Pagar.ContaPagar.strCod_Doc  := FieldByName('CPAG_GENE_ID_TIPO_DOC').AsString;
    fCad_Contas_Pagar.ContaPagar.strDoc      := FieldByName('TIPO_DOC').AsString;
    fCad_Contas_Pagar.ContaPagar.strObs      := FieldByName('CPAG_OBS').AsString;
    fCad_Contas_Pagar.ContaPagar.dtEmissao   := FieldByName('CPAG_DTA_EMISSAO').AsDateTime;
    fCad_Contas_Pagar.ContaPagar.dtVecto     := FieldByName('CPAG_DTA_VCTO').AsDateTime;
    fCad_Contas_Pagar.ContaPagar.floatVlr    := FieldByName('CPAG_VALOR_DOC').AsFloat;
    fCad_Contas_Pagar.ContaPagar.floatJuro   := FieldByName('CPAG_VALOR_JUROS').AsFloat;
    fCad_Contas_Pagar.ContaPagar.floatMora   := FieldByName('CPAG_VALOR_MORA').AsFloat;
  end;
end;


procedure TfContas_Pagar.FormCreate(Sender: TObject);
begin
  FormatSettings.ShortDateFormat := 'dd/mm/yyyy';
  cdsGeral     := dmContas_Pagar.cdsContas_Pagar;
  cdsGeral.Open;
  inherited;
  OpenClientDataSet(1);
  formCadastro   := fCad_Contas_Pagar;
  with fCad_Contas_Pagar do begin
    cdsLocal     := dmContas_Pagar.cdsContas_Pagar;
...
procedure TfContas_Pagar.frPesqContaBancariabtnPesquisarClick(Sender: TObject);
begin
  inherited;
...
procedure TfContas_Pagar.frPesqTipoDocckOmitirClick(Sender: TObject);
begin
  inherited;
...
procedure TfContas_Pagar.frPesqTipoDocedtCodigoExit(Sender: TObject);
var
  strTMP : String;
begin
  inherited;
...
procedure TfContas_Pagar.LancaDadosCartao(strTMP : String);
begin
...
procedure TfContas_Pagar.SetaValoresFrames;
var
  intI  : Integer;
begin
  for intI := 0 to ComponentCount - 1 do begin
...
procedure TfContas_Pagar.grdBasicoDrawColumnCell(Sender: TObject;
...
procedure TfContas_Pagar.OpenClientDataSet(intPos : Integer = 99);
begin
...
initialization
  RegisterClass(TfContas_Pagar);
finalization
  UnRegisterClass(TfContas_Pagar);
