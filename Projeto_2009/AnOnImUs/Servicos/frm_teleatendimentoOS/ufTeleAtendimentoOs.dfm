inherited fTeleAtendimentoOS: TfTeleAtendimentoOS
  Caption = 'fTeleAtendimentoOS'
  ClientWidth = 1294
  ExplicitWidth = 1300
  ExplicitHeight = 551
  PixelsPerInch = 96
  TextHeight = 13
  inherited panelInferior: TPanel
    Width = 1294
    ExplicitLeft = -8
    ExplicitTop = 473
    ExplicitWidth = 1294
    inherited btnCancelar: TSpeedButton
      Left = 347
      Top = 7
      ExplicitLeft = 347
      ExplicitTop = 7
    end
    inherited btnNovo: TSpeedButton
      Left = 347
      Top = 4
      Align = alNone
      Visible = False
      ExplicitLeft = 347
      ExplicitTop = 4
    end
    inherited btnEditar: TSpeedButton
      Left = 1
      Width = 200
      Caption = '&Realizar Atendimento'
      ExplicitLeft = 1
      ExplicitWidth = 200
    end
    inherited btnExcluir: TSpeedButton
      Left = 347
      Top = 4
      Align = alNone
      Visible = False
      ExplicitLeft = 347
      ExplicitTop = 4
    end
    inherited btnImprimir: TSpeedButton
      Left = 347
      Top = 4
      Align = alNone
      ExplicitLeft = 347
      ExplicitTop = 4
    end
    inherited btnLocalizar: TSpeedButton
      Left = 347
      Top = 4
      Align = alNone
      Visible = False
      ExplicitLeft = 347
      ExplicitTop = 4
    end
  end
  inherited panelSuperior: TPanel
    Width = 1294
    ExplicitWidth = 1294
    inherited btnHelp: TSpeedButton
      Left = 1093
      ExplicitLeft = 1113
    end
    inherited Label1: TLabel
      Width = 150
      Caption = 'Tele-Atendimento OS'
      ExplicitWidth = 150
    end
    inherited btnFecharRotina: TSpeedButton
      Left = 1193
      ExplicitLeft = 1203
    end
  end
  inherited gropGridPrincipal: TGroupBox
    Width = 1294
    ExplicitWidth = 1294
    inherited grdBasico: TDBGrid
      Top = 210
      Width = 1290
      Height = 216
    end
    object panelFiltros: TPanel
      Left = 2
      Top = 15
      Width = 1290
      Height = 195
      Align = alTop
      BevelOuter = bvNone
      Color = clWindow
      ParentBackground = False
      TabOrder = 1
      object btnFlat: TSpeedButton
        Left = 5
        Top = 0
        Width = 210
        Height = 24
        Caption = '- Clique aqui para expandir'
        Flat = True
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5}
        ParentFont = False
        OnClick = btnFlatClick
      end
      object Image3: TImage
        Left = 214
        Top = 4
        Width = 16
        Height = 16
        AutoSize = True
        Center = True
        Picture.Data = {
          
          F5F5}
        Transparent = True
        Visible = False
      end
      object Image4: TImage
        Left = 234
        Top = 4
        Width = 16
        Height = 16
        AutoSize = True
        Center = True
        Picture.Data = {
          
          1919}
        Transparent = True
        Visible = False
      end
      object btnFiltrar: TBitBtn
        Left = 996
        Top = 153
        Width = 95
        Height = 35
        Caption = '&Filtrar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          B683D5B683D5B683D5B683D5B684D5B480DAC095EBE0D1F5F8FC}
        ParentFont = False
        TabOrder = 9
        OnClick = btnFiltrarClick
      end
      object btnLimpar: TBitBtn
        Left = 1092
        Top = 153
        Width = 95
        Height = 35
        Caption = 'Li&mpar'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold]
        Glyph.Data = {
          
          B683D5B683D5B683D5B683D5B684D5B480DAC095EBE0D1F5F8FC}
        ParentFont = False
        TabOrder = 10
        OnClick = btnLimparClick
      end
      object lbDtCad: TGroupBox
        Left = 996
        Top = 68
        Width = 217
        Height = 38
        Caption = 'Dt. Atendimento'
        TabOrder = 8
        object edtDtCadInicio: TDateEdit
          Left = 6
          Top = 13
          Width = 100
          Height = 21
          NumGlyphs = 2
          TabOrder = 0
        end
        object edtDtCadFim: TDateEdit
          Left = 110
          Top = 13
          Width = 100
          Height = 21
          NumGlyphs = 2
          TabOrder = 1
        end
      end
      inline frPesqCliente: TfrPesqIndividual
        Left = 0
        Top = 26
        Width = 461
        Height = 40
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        ExplicitTop = 26
        ExplicitWidth = 461
        inherited Label2: TLabel
          Width = 33
          Caption = 'Cliente'
          Font.Charset = DEFAULT_CHARSET
          Font.Name = 'Tahoma'
          ParentFont = False
          ExplicitWidth = 33
        end
        inherited edtDescricao: TEdit
          Width = 282
          ExplicitWidth = 282
        end
        inherited rdOpcoes: TRadioGroup
          ItemIndex = 0
        end
        inherited ckOmitir: TCheckBox
          Left = 290
          Width = 135
          Visible = True
          ExplicitLeft = 290
          ExplicitWidth = 135
        end
      end
      inline frPesqAtendente: TfrPesqIndividual
        Left = 1
        Top = 67
        Width = 461
        Height = 40
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        ExplicitLeft = 1
        ExplicitTop = 67
        ExplicitWidth = 461
        inherited Label2: TLabel
          Width = 54
          Caption = 'Atendente'
          ExplicitWidth = 54
        end
        inherited btnPesquisar: TSpeedButton
          OnClick = frPesqAtendentebtnPesquisarClick
        end
        inherited edtDescricao: TEdit
          Width = 282
          ExplicitWidth = 282
        end
        inherited rdOpcoes: TRadioGroup
          ItemIndex = 22
        end
        inherited ckOmitir: TCheckBox
          Left = 290
          Width = 135
          Visible = True
          ExplicitLeft = 290
          ExplicitWidth = 135
        end
        inherited edtOpcao: TEdit
          Top = 3
          Width = 56
          ExplicitTop = 3
          ExplicitWidth = 56
        end
      end
      inline frPesqDuplo: TfrPesqDuplo
        Left = 467
        Top = 26
        Width = 521
        Height = 80
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        ExplicitLeft = 467
        ExplicitTop = 26
        ExplicitWidth = 521
        inherited frPesqPrimeiro: TfrPesqIndividual
          Width = 521
          ExplicitWidth = 521
          inherited Label3: TLabel
            Left = 445
            Visible = True
            ExplicitLeft = 445
          end
          inherited edtDescricao: TEdit
            Width = 265
            ExplicitWidth = 265
          end
          inherited rdOpcoes: TRadioGroup
            ItemIndex = 25
          end
          inherited ckOmitir: TCheckBox
            Left = 290
            Width = 135
            Visible = True
            ExplicitLeft = 290
            ExplicitWidth = 135
          end
          inherited edtOpcao: TEdit
            Left = 445
            Visible = True
            ExplicitLeft = 445
          end
        end
        inherited frPesqSegundo: TfrPesqIndividual
          Width = 521
          ExplicitWidth = 521
          inherited Label3: TLabel
            Left = 445
            Visible = True
            ExplicitLeft = 445
          end
          inherited edtDescricao: TEdit
            Width = 265
            ExplicitWidth = 265
          end
          inherited rdOpcoes: TRadioGroup
            ItemIndex = 24
          end
          inherited ckOmitir: TCheckBox
            Left = 290
            Width = 135
            Visible = True
            ExplicitLeft = 290
            ExplicitWidth = 135
          end
          inherited edtOpcao: TEdit
            Left = 445
            Visible = True
            ExplicitLeft = 445
          end
        end
      end
      inline frPesqTipoServico: TfrPesqIndividual
        Left = 0
        Top = 109
        Width = 461
        Height = 40
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        ExplicitTop = 109
        ExplicitWidth = 461
        inherited Label2: TLabel
          Width = 77
          Caption = 'Tipo de Servi'#231'o'
          ExplicitWidth = 77
        end
        inherited edtDescricao: TEdit
          Width = 282
          ExplicitWidth = 282
        end
        inherited rdOpcoes: TRadioGroup
          ItemIndex = 16
        end
        inherited ckOmitir: TCheckBox
          Left = 290
          Width = 135
          Visible = True
          ExplicitLeft = 290
          ExplicitWidth = 135
        end
        inherited edtOpcao: TEdit
          Width = 56
          ExplicitWidth = 56
        end
      end
      inline frPesqEquipamento: TfrPesqIndividual
        Left = 467
        Top = 109
        Width = 522
        Height = 40
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        ExplicitLeft = 467
        ExplicitTop = 109
        ExplicitWidth = 522
        inherited Label2: TLabel
          Width = 69
          Caption = 'Equipamento'
          ExplicitWidth = 69
        end
        inherited Label3: TLabel
          Left = 1500
          ExplicitLeft = 1500
        end
        inherited edtDescricao: TEdit
          Width = 346
          ExplicitWidth = 346
        end
        inherited rdOpcoes: TRadioGroup
          ItemIndex = 10
        end
        inherited ckOmitir: TCheckBox
          Left = 290
          Width = 135
          Visible = True
          ExplicitLeft = 290
          ExplicitWidth = 135
        end
        inherited edtOpcao: TEdit
          Left = 1500
          Width = 56
          ExplicitLeft = 1500
          ExplicitWidth = 56
        end
      end
      inline frPesqTecnicoOS: TfrPesqIndividual
        Left = 0
        Top = 149
        Width = 461
        Height = 40
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        ExplicitTop = 149
        ExplicitWidth = 461
        inherited Label2: TLabel
          Width = 62
          Caption = 'T'#233'cnico O.S.'
          ExplicitWidth = 62
        end
        inherited edtDescricao: TEdit
          Width = 282
          ExplicitWidth = 282
        end
        inherited rdOpcoes: TRadioGroup
          Top = 24
          ItemIndex = 11
          ExplicitTop = 24
        end
        inherited ckOmitir: TCheckBox
          Left = 290
          Width = 135
          Visible = True
          ExplicitLeft = 290
          ExplicitWidth = 135
        end
        inherited edtOpcao: TEdit
          Width = 56
          ExplicitWidth = 56
        end
      end
      object GroupBox1: TGroupBox
        Left = 996
        Top = 28
        Width = 217
        Height = 38
        Caption = 'Dt. Agendamento O.S.'
        TabOrder = 7
        object edtDtAgendInicio: TDateEdit
          Left = 6
          Top = 13
          Width = 100
          Height = 21
          NumGlyphs = 2
          TabOrder = 0
        end
        object edtDtAgendFim: TDateEdit
          Left = 110
          Top = 13
          Width = 100
          Height = 21
          NumGlyphs = 2
          TabOrder = 1
        end
      end
      inline frPesqTipoAgendamento: TfrPesqIndividual
        Left = 467
        Top = 149
        Width = 522
        Height = 40
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Segoe UI'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        ExplicitLeft = 467
        ExplicitTop = 149
        ExplicitWidth = 522
        inherited Label2: TLabel
          Width = 98
          Caption = 'Tipo Agendamento'
          ExplicitWidth = 98
        end
        inherited Label3: TLabel
          Left = 1500
          ExplicitLeft = 1500
        end
        inherited edtDescricao: TEdit
          Width = 346
          ExplicitWidth = 346
        end
        inherited rdOpcoes: TRadioGroup
          ItemIndex = 32
        end
        inherited ckOmitir: TCheckBox
          Left = 290
          Width = 135
          Visible = True
          ExplicitLeft = 290
          ExplicitWidth = 135
        end
        inherited edtOpcao: TEdit
          Left = 1500
          Width = 56
          ExplicitLeft = 1500
          ExplicitWidth = 56
        end
      end
    end
  end
  inherited dsGrid: TDataSource
    DataSet = dmTeleAtendimentoOS.cdsClientes
  end
  inherited frRelatorio: TfrxReport
    Left = 928
    Top = 328
    Datasets = <>
    Variables = <>
    Style = <>
    inherited Page1: TfrxReportPage
      inherited PageHeader1: TfrxPageHeader
        inherited Memo15: TfrxMemoView
          Memo.UTF8W = (
            'C'#243'digo')
        end
      end
      inherited PageFooter1: TfrxPageFooter
        inherited Memo1: TfrxMemoView
          Memo.UTF8W = (
            'www.anonimus.info')
        end
        inherited SysMemo1: TfrxSysMemoView
          Memo.UTF8W = (
            '[PAGE#] de [TOTALPAGES#]')
        end
      end
      inherited ReportTitle1: TfrxReportTitle
        inherited lblFantasia: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
        inherited SysMemo2: TfrxSysMemoView
          Memo.UTF8W = (
            '[DATE]')
        end
        inherited SysMemo3: TfrxSysMemoView
          Memo.UTF8W = (
            '[TIME]')
        end
        inherited Memo3: TfrxMemoView
          Memo.UTF8W = (
            'Data/Hora: ')
        end
        inherited Memo4: TfrxMemoView
          Memo.UTF8W = (
            'Usu'#225'rio:')
        end
        inherited lbTit01: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbTit02: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbTit03: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lblUsuario: TfrxMemoView
          Memo.UTF8W = (
            'Usu'#225'rio:')
        end
        inherited lbFild01: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbFild02: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbFild03: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lblEndereco: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
        inherited lblRazao_Social: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
        inherited Memo14: TfrxMemoView
          Memo.UTF8W = (
            'T'#205'TULO DO RELAT'#211'RIO')
        end
        inherited lblTelefone: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
      end
    end
  end
  inherited dstRelatorio: TfrxDBDataset
    Left = 928
    Top = 280
  end
  object Timer2: TTimer
    Interval = 60000
    OnTimer = Timer2Timer
    Left = 864
    Top = 280
  end
end
