inherited fTransf_Servicos: TfTransf_Servicos
  Caption = 'fTransf_Servicos'
  ExplicitWidth = 1050
  ExplicitHeight = 551
  PixelsPerInch = 96
  TextHeight = 13
  inherited panelSuperior: TPanel
    inherited Label1: TLabel
      Width = 369
      Caption = 'Transfer'#234'ncia de Servi'#231'os/Pe'#231'as entre Colaboradores'
      ExplicitWidth = 369
    end
  end
  inherited gropGridPrincipal: TGroupBox
    Top = 194
    Height = 273
    ExplicitTop = 194
    ExplicitHeight = 273
    inherited grdBasico: TDBGrid
      Height = 256
    end
  end
  object panelFiltros: TPanel [3]
    Left = 0
    Top = 39
    Width = 1044
    Height = 155
    Align = alTop
    BevelOuter = bvNone
    Color = clWindow
    ParentBackground = False
    TabOrder = 3
    object btnFlat: TSpeedButton
      Left = 5
      Top = 1
      Width = 210
      Height = 25
      Caption = '- Clique aqui para expandir'
      Flat = True
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5F5}
      ParentFont = False
      OnClick = btnFlatClick
    end
    object Image3: TImage
      Left = 214
      Top = 4
      Width = 16
      Height = 16
      AutoSize = True
      Center = True
      Picture.Data = {
        
        F5F5}
      Transparent = True
      Visible = False
    end
    object Image4: TImage
      Left = 234
      Top = 4
      Width = 16
      Height = 16
      AutoSize = True
      Center = True
      Picture.Data = {
        
        1919}
      Transparent = True
      Visible = False
    end
    object lbDtInstalacao: TGroupBox
      Left = 483
      Top = 69
      Width = 217
      Height = 38
      Caption = 'Dt. Instala'#231#227'o'
      TabOrder = 4
      object edtDtInstalInicio: TDateEdit
        Left = 5
        Top = 13
        Width = 100
        Height = 21
        NumGlyphs = 2
        TabOrder = 0
      end
      object edtDtInstalFim: TDateEdit
        Left = 110
        Top = 13
        Width = 100
        Height = 21
        NumGlyphs = 2
        TabOrder = 1
      end
    end
    object btnFiltrar: TBitBtn
      Left = 509
      Top = 113
      Width = 95
      Height = 35
      Caption = '&Filtrar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        B683D5B683D5B683D5B683D5B684D5B480DAC095EBE0D1F5F8FC}
      ParentFont = False
      TabOrder = 5
      OnClick = btnFiltrarClick
    end
    object BitBtn2: TBitBtn
      Left = 605
      Top = 113
      Width = 95
      Height = 35
      Caption = 'Li&mpar'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        B683D5B683D5B683D5B683D5B684D5B480DAC095EBE0D1F5F8FC}
      ParentFont = False
      TabOrder = 6
      OnClick = BitBtn2Click
    end
    object lbDtCad: TGroupBox
      Left = 483
      Top = 27
      Width = 217
      Height = 38
      Caption = 'Dt. Cadastro'
      TabOrder = 3
      object edtDtCadInicio: TDateEdit
        Left = 6
        Top = 13
        Width = 100
        Height = 21
        NumGlyphs = 2
        TabOrder = 0
      end
      object edtDtCadFim: TDateEdit
        Left = 110
        Top = 13
        Width = 100
        Height = 21
        NumGlyphs = 2
        TabOrder = 1
      end
    end
    inline frPesqCliente: TfrPesqIndividual
      Left = 2
      Top = 25
      Width = 475
      Height = 40
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      ExplicitLeft = 2
      ExplicitTop = 25
      ExplicitWidth = 475
      inherited Label2: TLabel
        Width = 36
        Caption = 'Cliente'
        ExplicitWidth = 36
      end
      inherited edtDescricao: TEdit
        Width = 300
        ExplicitWidth = 300
      end
      inherited rdOpcoes: TRadioGroup
        ItemIndex = 0
      end
      inherited ckOmitir: TCheckBox
        Left = 343
        Width = 131
        Visible = True
        ExplicitLeft = 343
        ExplicitWidth = 131
      end
    end
    inline frPesqTecDestino: TfrPesqIndividual
      Left = 2
      Top = 108
      Width = 475
      Height = 40
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      ExplicitLeft = 2
      ExplicitTop = 108
      ExplicitWidth = 475
      inherited Label2: TLabel
        Width = 81
        Caption = 'T'#233'cnico Destino'
        ExplicitWidth = 81
      end
      inherited edtDescricao: TEdit
        Width = 300
        ExplicitWidth = 300
      end
      inherited rdOpcoes: TRadioGroup
        ItemIndex = 11
      end
      inherited ckOmitir: TCheckBox
        Left = 343
        Width = 131
        Visible = True
        ExplicitLeft = 343
        ExplicitWidth = 131
      end
    end
    inline frPesqTecOrigem: TfrPesqIndividual
      Left = 2
      Top = 66
      Width = 475
      Height = 40
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Segoe UI'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      ExplicitLeft = 2
      ExplicitTop = 66
      ExplicitWidth = 475
      inherited Label2: TLabel
        Width = 79
        Caption = 'T'#233'cnico Origem'
        ExplicitWidth = 79
      end
      inherited edtDescricao: TEdit
        Width = 300
        ExplicitWidth = 300
      end
      inherited rdOpcoes: TRadioGroup
        ItemIndex = 11
      end
      inherited ckOmitir: TCheckBox
        Left = 343
        Width = 131
        Visible = True
        ExplicitLeft = 343
        ExplicitWidth = 131
      end
    end
  end
  inherited dsGrid: TDataSource
    Left = 464
  end
  inherited Timer1: TTimer
    Left = 496
  end
  inherited frRelatorio: TfrxReport
    Datasets = <>
    Variables = <>
    Style = <>
    inherited Page1: TfrxReportPage
      inherited PageHeader1: TfrxPageHeader
        inherited Memo15: TfrxMemoView
          Memo.UTF8W = (
            'C'#243'digo')
        end
      end
      inherited PageFooter1: TfrxPageFooter
        inherited Memo1: TfrxMemoView
          Memo.UTF8W = (
            'www.anonimus.info')
        end
        inherited SysMemo1: TfrxSysMemoView
          Memo.UTF8W = (
            '[PAGE#] de [TOTALPAGES#]')
        end
      end
      inherited ReportTitle1: TfrxReportTitle
        inherited lblFantasia: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
        inherited SysMemo2: TfrxSysMemoView
          Memo.UTF8W = (
            '[DATE]')
        end
        inherited SysMemo3: TfrxSysMemoView
          Memo.UTF8W = (
            '[TIME]')
        end
        inherited Memo3: TfrxMemoView
          Memo.UTF8W = (
            'Data/Hora: ')
        end
        inherited Memo4: TfrxMemoView
          Memo.UTF8W = (
            'Usu'#225'rio:')
        end
        inherited lbTit01: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbTit02: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbTit03: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lblUsuario: TfrxMemoView
          Memo.UTF8W = (
            'Usu'#225'rio:')
        end
        inherited lbFild01: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbFild02: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lbFild03: TfrxMemoView
          Memo.UTF8W = (
            '...')
        end
        inherited lblEndereco: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
        inherited lblRazao_Social: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
        inherited Memo14: TfrxMemoView
          Memo.UTF8W = (
            'T'#205'TULO DO RELAT'#211'RIO')
        end
        inherited lblTelefone: TfrxMemoView
          Memo.UTF8W = (
            'Empresa:')
        end
      end
    end
  end
  inherited funcBasico: TFuncoes
    Left = 432
  end
end
