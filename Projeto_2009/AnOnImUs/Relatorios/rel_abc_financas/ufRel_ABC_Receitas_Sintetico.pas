unit ufRel_ABC_Receitas_Sintetico;
...
type
  TfRel_ABC_Receitas_Sintetico = class(TfImpressoes)
    lbID: TQRLabel;
...
var
  fRel_ABC_Receitas_Sintetico: TfRel_ABC_Receitas_Sintetico;
  floatTotal,
  floatDebito,
  floatCredito : Real;
...
procedure TfRel_ABC_Receitas_Sintetico.QRBand4BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
var
  floatPerc : Real;
begin
  inherited;
  floatTotal := floatTotal + dmRel_Abc_Financas.cdsRel_ABC_Receitas.FieldByName('VALOR').AsFloat;
  floatPerc  := (floatDebito / 100);
  floatPerc  := (dmRel_Abc_Financas.cdsRel_ABC_Receitas.FieldByName('VALOR').AsFloat / floatPerc);
...
procedure TfRel_ABC_Receitas_Sintetico.QRBand5BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  inherited;
  ...
  lbDeb.Caption  := 'R$ ' + FormatFloat('###,###,##0.00',(floatDebito));
  lbLiq.Caption  := 'R$ ' + FormatFloat('###,###,##0.00',(floatCredito - (floatDebito)));
end;

procedure TfRel_ABC_Receitas_Sintetico.qrepRelatorioBeforePrint(
...
function TfRel_ABC_Receitas_Sintetico.TotalCredito(intVisual: Integer): Real;
var
  qryCons : TSQLQuery;
begin
  try
...
function TfRel_ABC_Receitas_Sintetico.TotalDebito(intVisual: Integer): Real;
var
  qryCons : TSQLQuery;
begin
  try
